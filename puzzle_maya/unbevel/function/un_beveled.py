...
def linePlaneIntersection(C, NormalFace, A, B):
    '''
    Description:
...
def lineIntersectForBevelEdges(A, B, C, D):
    '''
...
def twoEdgesIntersect(edgesSel):
    '''
...
def getEdgeConnectToHardEdges(currentEdgeSel, *arg):
        ...
        for setEdgesConnected in edgeSets:
            #try:
                ...
                if len(edgesExpand) == 2:
                    ...
                    # get the intersect Point base on 4 point
                    intersectPoint = lineIntersectForBevelEdges(pointA, pointB, pointC, pointD)
                    if intersectPoint == 0:            
                        ...
                        listHardEdges += getEdgeConnectToHardEdges(setEdgeOld)
                    ...
                    if faceToIntersect:
                        ...
                        for face in trianFace:
                            ...
                            if vertexOfFace[0] in listVert:
                                NormalFaceIntersect.append(queryFaceNormal(face))
                        ...
                        for i in range(0, len(NormalFaceIntersect)):
                            ...
                            intersectPoint = linePlaneIntersection(pointC, NormalFace, pointA, pointB)
                ...
                if len(edgesExpand) == 0:
                   # print 'th3'
                    ...
                    allIntersectPoint = [twoEdgesIntersect(i) for i in allPairEdges if twoEdgesIntersect(i) != 0]
...
def unBevelUI(*arg):
    ...
    mainWindow= cmds.window(windowID,title='TestUnbevel', resizeToFitChildren=True)
    tabUI = cmds.rowColumnLayout(width=Unit_UIWidth+20, columnOffset=[1,'both',10],rowOffset=([2,'bottom',15],[1,'both',10]))

    cmds.rowColumnLayout(numberOfColumns=1, columnWidth=[(1, Unit_UIWidth)], rowOffset=([5,'top',5]))
    
    cmds.text(l='Select Edges beveled to Unbevel', h=20,fn="boldLabelFont",align='left')
    cmds.setParent('..')
    cmds.button(l="Unbevel ", bgc =[0.2, 0.3, 0.4], c=smartCollapsing)
    cmds.setParent('..')  
    cmds.setParent('..')
