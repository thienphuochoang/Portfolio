...
def GetUVsShell(mObject, uv_set):
    """
     [in] mObject
    ...
    for i, n in enumerate(uvShellIds[1]):
        ...
        if n in shells:
            # print n
            # print mFnMesh.getUV(i, "map1")
            shells[n].append({"{}.map[{}]".format(fullPathObj, i) : mFnMesh.getUV(i, uv_set)})
        else:
            shells[n] = [{"{}.map[{}]".format(fullPathObj, i) : mFnMesh.getUV(i, uv_set)}]
    return shells


def get_uv_shell_overlap(uv_set):
    cmds.polyUVSet(currentUVSet=True,  uvSet=uv_set)
    """error if in a uv's shell has the vertices overlap"""
    bad_list = []
    ...
    while not allNodes.isDone(): 
        ...
        dagObjectCheck = dag_objects_do_not_check([fullPathObj], exclude_objs)
        cmds.polyUVSet(dagObjectCheck, currentUVSet=True,  uvSet=uv_set)
        if dagObjectCheck:
            shellUVs = GetUVsShell(mObject, uv_set)
...
def result():
    """function required"""
    result_error_list_map1 = get_uv_shell_overlap("map1")
    ...
    if result_error_list_map1:
        for er in result_error_list_map1:
            error_dict[er] = "---Map1 has vertex map overlaping. :("
    else: 
